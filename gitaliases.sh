...
pull () { local HEADHASH=`git describe --always --abbrev=40`; git pull --rebase $*; git submodule update; echo; PAGER='cat -B' git log --format="%C(yellow)%h %C(green)%an%C(reset): %s" $HEADHASH.. | sed -nr 's/([^:]+)\:/\1\t/;p'; }
